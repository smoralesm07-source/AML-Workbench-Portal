'use strict';
/* ATLAS AML · IGR v2A production authority 0.91.2
 ...
 * Presentation: seamless ATLAS host; no nested app/window chrome.
 */
(function atlasIgrV2AAuthority0912(){
  ...
  const IFRAME='./assets/territorio-aml-beta.html?v=0912-1';
  ...
  function ensureHostStyle(){
    ...
    s.textContent=`
      ...
      [data-atlas-igr-v2a-host] iframe{display:block!important;width:100%!important;max-width:none!important;height:1200px;border:0!important;border-radius:0!important;overflow:hidden!important;background:transparent!important;box-shadow:none!important}
    `;
  ...
  function outerTokens(){
    ...
    return {bg:read('--atlas-bg','#07111f'),panel:read('--atlas-surface','#0c1728'),panel2:read('--atlas-surface-2','#101d30'),line:read('--atlas-border','rgba(148,163,184,.18)'),text:read('--atlas-text','#e8eef7'),muted:read('--atlas-muted','#9aa8bc'),accent:read('--atlas-accent','#f28c28')};
  }
  ...
  function applySeamlessFrame(frame){
    try{
      ...
      r.style.setProperty('--bg',t.bg);r.style.setProperty('--panel',t.panel);r.style.setProperty('--panel2',t.panel2);r.style.setProperty('--line',t.line);r.style.setProperty('--text',t.text);r.style.setProperty('--muted',t.muted);r.style.setProperty('--cyan',t.accent);
      ...
      style.textContent=`
        ...
        body>.shell{width:100%!important;max-width:none!important;margin:0!important;padding:0!important}
        body>.shell>.header{display:none!important}
        .controls{margin-top:0!important}
        .card{box-shadow:none!important}
        #map{min-height:620px}
        @media(max-width:1200px){#map{min-height:520px}}
      `;
  ...
  window.ATLAS_TERRITORY_THREAT_CEAD_V1={source:SOURCE,scoreVersion:'1.0.0',topLevelWeight:1,aggregation:'IGR_V2A_CEAD_LA_AUTHORITY',replacedLegacy15Percent:true,seamlessAtlasHost:true};
