'use strict';
/* ATLAS AML · Universo SO canonical loader 0.81.7
 * Mantiene el path histórico cargado por index.html, pero delega toda la
 * autoridad de Universo SO a la implementación 0.81.7. Sin observers globales.
 * Contexto SII se carga sólo como enriquecimiento de la Ficha 360.
 * OSFL es una condición nativa y filtrable sustentada por el contrato de datos.
 */
(function atlasUniversoSOCanonicalLoader0817(){
  if(window.AtlasUniversoSO0816)return;
  const css='./assets/atlas-universo-so-entity-explorer-0816.css?v=0816-1';
  const js='./assets/atlas-universo-so-entity-explorer-0816.js?v=0817-1';
  ...
  const osflCss='./assets/atlas-universo-so-osfl-context-0842.css?v=0842-1';
  ...
  if(!document.querySelector('link[data-atlas-universo-osfl-0842]')){
    const link=document.createElement('link');link.rel='stylesheet';link.href=osflCss;link.dataset.atlasUniversoOsfl0842='1';document.head.appendChild(link);
  }
  let resolveReady,rejectReady;
  const ready=new Promise((resolve,reject)=>{resolveReady=resolve;rejectReady=reject;});
  const proxy={version:'0.81.7',authority:'ENTITY_EXPLORER_SPECIALIZATION_0816_LOADING',open:(mode='inscritos')=>ready.then(api=>api.open(mode)),state:()=>({loading:true,version:'0.81.7'})};
  ...
  script.onload=()=>{
    ...
    if(api?.authority==='ENTITY_EXPLORER_SPECIALIZATION_0816'){
      ...
      resolveReady(api);
    }else rejectReady(new Error('Autoridad Universo SO 0.81.7 no instalada'));
  };
  script.onerror=()=>rejectReady(new Error('No fue posible cargar Universo SO 0.81.7'));
