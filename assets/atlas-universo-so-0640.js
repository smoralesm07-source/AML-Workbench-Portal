...
(function atlasUniversoSO0640(){
  ...
  function cleanLegacyCards(){
    document.querySelectorAll('.uso61-truth,.uso63-scope').forEach(n=>n.remove());
  }
  ...
  async function loadTiers(){
    if(tierCache||loading)return tierCache;
    const client=db();if(!client)return null;
    loading=true;
    try{
      const {data,error}=await client.from('aml_uaf_potential_subject_snapshot')
        .select('detection_tier,is_actionable');
      if(error)throw error;
      const rows=data||[];
      const out={total:rows.length,actionable:0,A_ALTA:0,B_MEDIA:0,C_EXPLORATORIA:0};
      for(const r of rows){
        if(r.is_actionable)out.actionable++;
        if(out[r.detection_tier]!==undefined)out[r.detection_tier]++;
      }
      tierCache=out;return out;
    }catch(_e){return null;}finally{loading=false;}
  }

  function tierHtml(t){
    return `<section class="uso64-tier-strip">
      <h3>Universo ampliado de potenciales SO</h3>
      <p>El motor 0.64 privilegia cobertura: el tipo de entidad dejó de ser un filtro excluyente y pasa a ponderar la evidencia.</p>
      <div class="uso64-tiers">
        <div class="uso64-tier total"><b>${fmt(t.total)}</b><span>Potenciales detectados · ${fmt(t.actionable)} accionables</span></div>
        <div class="uso64-tier"><b>${fmt(t.A_ALTA)}</b><span>Nivel A · evidencia alta</span></div>
        <div class="uso64-tier"><b>${fmt(t.B_MEDIA)}</b><span>Nivel B · evidencia media</span></div>
        <div class="uso64-tier"><b>${fmt(t.C_EXPLORATORIA)}</b><span>Nivel C · exploración</span></div>
      </div>
      <div class="uso64-note">Potencial SO = hipótesis de fiscalización. No equivale a sujeto obligado no inscrito ni a incumplimiento.</div>
    </section>`;
  }

  async function patchPotential(){
    const host=document.querySelector('#so-potential');
    if(!host||host.querySelector('.uso64-tier-strip'))return;
    const t=await loadTiers();if(!t||!document.contains(host))return;
    const kpis=host.querySelector('.so-kpis');
    if(kpis)kpis.insertAdjacentHTML('afterend',tierHtml(t));
    else host.insertAdjacentHTML('afterbegin',tierHtml(t));
  }
