...
(function atlasOperationalRecovery0805(){
  ...
  function publish(status,extra={}){window.__ATLAS_OPERATIONAL_RECOVERY_0704__={active:true,version:VERSION,release:RELEASE,build:BUILD,status,radar:!!(fn('v019LoadOverview')||fn('loadOverview')),territory:!!fn('v019LoadTerritory'),osfl:!!fn('v030LoadOsfl'),questions:!!fn('v019LoadQuestions'),entities:!!window.__ATLAS_ENTITY_ENTRY__,universo080:!!window.AtlasUniversoSO0800,publicSpend:!!window.AtlasPublicSpendV2,indicatorMethodology:!!window.AtlasIndicatorMethodologyV1,routePolicy:'SINGLE_CAPTURE_FAST_TRANSITION',startupPolicy:'ONE_VISIBLE_RENDER_AFTER_FINAL_RUNTIME',freezeGuard:'NO_BODY_TREEWALK_NO_FORCED_SOURCE_REFRESH:DEFER_NONCRITICAL_UI_AND_SOURCE_HEALTH',checkedAt:new Date().toISOString(),...extra};}
  ...
  function deferredHousekeeping(){idle(()=>{try{ensureMenu();ensureIndicatorMethodology();window.AtlasIndicatorMethodologyV1?.refresh?.();window.AtlasCurrentUI?.refresh?.();window.AtlasGlobalSourceHealth?.schedule?.();}catch{}});}
  ...
  window.addEventListener('pageshow',()=>{neutralizeLegacyStartupVersion();installShellHook();ensureMenu();ensureIndicatorMethodology();enforceRelease();deferredHousekeeping();});
  window.addEventListener('atlas:nav-refresh',()=>requestAnimationFrame(()=>{ensureMenu();window.AtlasIndicatorMethodologyV1?.refresh?.();}));
  window.addEventListener('atlas:public-spend-v2-ready',()=>{ensureMenu();window.AtlasIndicatorMethodologyV1?.refresh?.();});
  const API={open,openTerritory,openOsfl,openUniverso,openEntities,ensureMenu,ensureIndicatorMethodology,refreshUniversoState,enforceRelease,neutralizeLegacyStartupVersion,health:()=>window.__ATLAS_OPERATIONAL_RECOVERY_0704__};
  ...
  neutralizeLegacyStartupVersion();installShellHook();enforceRelease();ensureIndicatorMethodology();requestAnimationFrame(()=>ensureMenu());deferredHousekeeping();publish('installed');
})();