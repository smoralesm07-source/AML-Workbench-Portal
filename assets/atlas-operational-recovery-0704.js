...
(function atlasOperationalRecovery0805(){
  ...
  const VERSION='0.80.5',RELEASE='0.70.5',VIEW_SO='sujetos-obligados',VIEW_ENTITIES='entities',VIEW_TERRITORY='territory',VIEW_OSFL='osfl';
  let opening=false,lastView='';
  ...
  function enforceRelease(){window.AtlasRelease?.apply?.();window.__ATLAS_RELEASE__={...(window.__ATLAS_RELEASE__||{}),version:RELEASE,build:'0705',active:true,freezeGuard:'TARGETED_ONLY',checkedAt:new Date().toISOString()};}
  function neutralizeLegacyStartupVersion(){const noop=()=>{};try{if(typeof v024ApplyVersion==='function')v024ApplyVersion=noop;}catch{}try{if(typeof v0211ApplyVersion==='function')v0211ApplyVersion=()=>enforceRelease();}catch{}window.__ATLAS_STARTUP_AUTHORITY_0804__={active:true,version:VERSION,legacyV024VersionWriter:false,renderGate:'BODY_AML_RUNTIME_READY',policy:'ONE_VISIBLE_RENDER_AFTER_FINAL_RUNTIME',installedAt:new Date().toISOString()};}
  function publish(status,extra={}){window.__ATLAS_OPERATIONAL_RECOVERY_0704__={active:true,version:VERSION,status,radar:!!(fn('v019LoadOverview')||fn('loadOverview')),territory:!!fn('v019LoadTerritory'),osfl:!!fn('v030LoadOsfl'),questions:!!fn('v019LoadQuestions'),entities:!!window.__ATLAS_ENTITY_ENTRY__,universo080:!!window.AtlasUniversoSO0800,publicSpend:!!window.AtlasPublicSpendV2,routePolicy:'SINGLE_CAPTURE_FAST_TRANSITION',startupPolicy:'ONE_VISIBLE_RENDER_AFTER_FINAL_RUNTIME',freezeGuard:'DEFER_NONCRITICAL_UI_AND_SOURCE_HEALTH',checkedAt:new Date().toISOString(),...extra};}
  ...
  function deferredHousekeeping(){idle(()=>{try{ensureMenu();window.AtlasCurrentUI?.refresh?.();window.AtlasGlobalSourceHealth?.schedule?.();}catch{}});}
  ...
  async function openOsfl(){setState(VIEW_OSFL);const loader=fn('v030LoadOsfl');if(!loader)throw new Error('Loader OSFL canónico no disponible');const result=await loader();window.__ATLAS_OSFL_CURRENT__={release:RELEASE,build:'0705',authority:'OSFL_CANONICAL_ROUTE_0801',entityRuntime:'aml_osfl_entity_runtime_snapshot',regionRuntime:'aml_osfl_region_runtime_snapshot',renderedAt:new Date().toISOString()};return result;}
  async function openEntities(){setState(VIEW_ENTITIES);const entry=window.__ATLAS_ENTITY_ENTRY__;if(!entry||typeof entry.load!=='function')throw new Error('Explorador de Entidades canónico no disponible');const result=await entry.load();window.__ATLAS_ENTITY360_CURRENT__={...(window.__ATLAS_ENTITY360_CURRENT__||{}),release:RELEASE,build:'0705',authority:'ENTITY_EXPLORER_0512_CANONICAL_ROUTE',mode:'idle',routeAuthority:'OPERATIONAL_RECOVERY_0805',renderedAt:new Date().toISOString()};return result;}
  async function open(view,source='delegated-click'){
    ...
    try{
      ...
      publish('ready',{view,source});deferredHousekeeping();return result===undefined?true:result;
  ...
  function installShellHook(){const current=window.shell;if(typeof current!=='function'||current.__atlas0805Shell)return;const wrapped=function(...args){const out=current.apply(this,args);requestAnimationFrame(()=>ensureMenu());return out;};Object.defineProperty(wrapped,'__atlas0805Shell',{value:true});window.shell=wrapped;}
  ...
  window.addEventListener('pageshow',()=>{neutralizeLegacyStartupVersion();installShellHook();ensureMenu();enforceRelease();deferredHousekeeping();});
  window.addEventListener('atlas:nav-refresh',()=>requestAnimationFrame(()=>ensureMenu()));
  window.addEventListener('atlas:public-spend-v2-ready',()=>ensureMenu());
  const API={open,openTerritory,openOsfl,openUniverso,openEntities,ensureMenu,refreshUniversoState,enforceRelease,neutralizeLegacyStartupVersion,health:()=>window.__ATLAS_OPERATIONAL_RECOVERY_0704__};
  ...
  neutralizeLegacyStartupVersion();installShellHook();enforceRelease();requestAnimationFrame(()=>ensureMenu());deferredHousekeeping();publish('installed');
})();