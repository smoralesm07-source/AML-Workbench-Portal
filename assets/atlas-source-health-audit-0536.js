...
(function atlasSourceHealthAudit0536(){
  const VERSION='SOURCE-HEALTH-AUDIT-0536.3',TTL=5*60*1000;
  ...
  const freshness=()=>window.AtlasDataAudit?.getState?.()?.detail?.sources||[];
  ...
  function catalog(){
    const rows=freshness().map(r=>({kind:'freshness',label:r.label||r.source_system||r.source_id||'Fuente',system:r.source_system||'ATLAS',latest:r.latest_record_label||day(r.latest_record_at||r.last_capture_at),checked:dt(r.last_checked_at),tone:freshnessTone(r),state:String(r.status||'SIN ESTADO').toUpperCase(),reason:r.reason||'Sin detalle'}));
  ...
  function summaryHtml(){
    ...
    const coverage=pending?'—':`${Number(o.coverage_effective_pct??0)}%`,operational=pending?(loadState==='loading'?'calculando disponibilidad…':'telemetría no disponible'):`${Number(o.healthy||0)} operativas · ${Number(o.watch||0)} revisar · ${Number(o.degraded||0)} caídas`,critical=pending?'—':Number(o.critical_down||0);
  ...
  function bind(rootEl){rootEl.querySelector('[data-ash-toggle]')?.addEventListener('click',e=>{const b=e.currentTarget,d=rootEl.querySelector('[data-ash-detail]'),open=b.getAttribute('aria-expanded')==='true';b.setAttribute('aria-expanded',String(!open));d.hidden=open;});rootEl.querySelectorAll('[data-ash-tab]').forEach(b=>b.addEventListener('click',()=>{activeTab=b.dataset.ashTab;rootEl.querySelectorAll('[data-ash-tab]').forEach(x=>x.classList.toggle('active',x===b));updateRoot();}));rootEl.querySelector('[data-ash-refresh]')?.addEventListener('click',async e=>{const b=e.currentTarget;b.disabled=true;b.textContent='Actualizando…';try{await Promise.allSettled([loadOps(true),loadRes(true)]);}finally{b.disabled=false;b.textContent='Actualizar';updateRoot();}});}
  async function startup(){mount();void loadRes(true);const waits=[0,400,1000,2000,4000];for(const wait of waits){if(wait)await new Promise(r=>setTimeout(r,wait));try{await loadOps(true);return;}catch{mount();}}updateRoot();}
  ...
  setInterval(()=>{void loadOps(true).catch(()=>{});void loadRes(true);},TTL);
  window.AtlasSourceHealthAudit={version:VERSION,refresh:()=>Promise.allSettled([loadOps(true),loadRes(true)]),getOperationalState:()=>ops,getResState:()=>resState,getLoadState:()=>({state:loadState,error:lastError?.message||null})};
})();
