...
function atlasVe2Rank(rows,getter){
  return atlasVe2PercentilePairs(rows.map((r,i)=>({key:i,value:getter(r)})));
}
function atlasVe2EnrichLevel(rows,computed,cfg){
  ...
  rows.forEach((r,i)=>{
    ...
    const sp=salesPct.get(i)??50,lp=laborPct.get(i)??50,ew=cfg.formula?.economic_materiality_sii||{};
    const econ=(Number(ew.sales_band_percentile)||0.60)*sp+(Number(ew.workers_per_active_entity_percentile)||0.40)*lp;
...
function atlasVe2Decorate(){
  ...
  const row=atlasVe2Selected();if(panel&&row){
    const p=panel.querySelector('.v032-card-head p');if(p)p.textContent='V/E v2 pondera la presencia potencial SII por un perfil sectorial enriquecido con riesgo estructural, ENR Chile, convertibilidad histórica de ROS y materialidad GAFILAT; añade 15% de escala económica territorial SII.';
    const grid=panel.querySelector('.v032-sector-grid');if(grid&&!panel.querySelector('.atlas-ve2-breakdown')){
      const v=row.meta.ve_v2||{};
      grid.insertAdjacentHTML('beforebegin',`<div class="atlas-ve2-breakdown"><div><span>V/E final</span><b>${atlasVe2Fmt(row.parts.vulnerability,1)}</b></div><div><span>Núcleo sectorial · 85%</span><b>${atlasVe2Fmt(v.sector_core,1)}</b></div><div><span>Escala económica SII · 15%</span><b>${atlasVe2Fmt(v.economic_materiality,1)}</b></div><div><span>Ventas · percentil</span><b>${atlasVe2Fmt(v.sales_percentile,0)}</b></div><div><span>Empleo/entidad · percentil</span><b>${atlasVe2Fmt(v.labor_percentile,0)}</b></div></div>`);
    }
    ...
    const footer=panel.querySelector('footer');if(footer)footer.textContent='V/E es una priorización territorial comparativa. ENR y GAFILAT se adaptan a escala sectorial de forma explícita; ICR es histórico y suavizado por tamaño; actividad SII no acredita condición jurídica de SO ni sospecha.';
  }
  const note=document.querySelector('.v032-method-note span');if(note&&!note.textContent.includes('V/E v2'))note.textContent+=' V/E v2 conserva el 45% superior, pero enriquece su interior con ENR Chile, ICR, materialidad sectorial GAFILAT y escala económica SII.';
}
...
if(typeof atlasVe2BaseTerritory==='function'){
  window.v019LoadTerritory=async function(...args){
