'use strict';
/* ATLAS AML · Reportabilidad actual + clasificación IIR × IRAR × ICE. */
(function atlasReportabilityIrar(){
  const core=window.ATLAS_IRAR_CURRENT;
  const UI_VERSION='IRAR-UI-1.8';
  ...
  try{
    ...
    V036_HELP.irar={t:'IRAR · Rendimiento Analítico de ROS ajustado por evidencia',b:'IRAR es el indicador de rendimiento de reportabilidad. Parte del rendimiento observado (ROS con indicios 2021–2025 / ROS enviados 2021–2025) y lo suaviza hacia una referencia leave-one-out de sectores comparables mediante un prior de 100 ROS. Con poca evidencia, el resultado se acerca a la referencia; con mayor volumen converge al dato observado. Se mantiene deliberadamente separado de IRAR-E: IRAR no es riesgo sectorial, riesgo individual ni probabilidad de LA/FT.'};
    V036_HELP.irar_e={t:'IRAR-E · Riesgo inherente sectorial',b:'IRAR-E pertenece al dominio de riesgo sectorial y combina vulnerabilidad estructural, materialidad y amenaza LA/FT/FP. No interviene en las coordenadas de este mapa de reportabilidad. Se usa como una capa separada cuando ATLAS construye priorización de fiscalización con cobertura y ponderadores aprobados.'};
    ...
    V036_HELP.irar_relative={t:'IRAR relativo',b:'Compara el IRAR ajustado del sector con el rendimiento esperado de su familia comparable. 1,0× equivale a la referencia; sobre 1,25× se considera rendimiento relativo alto y bajo 0,80× rendimiento relativo bajo. La lectura debe combinarse siempre con ICE.'};
    ...
    V036_HELP.reportability_profile={t:'Señal de revisión · IIR × IRAR × ICE',b:'Combina tres dimensiones propias de reportabilidad: intensidad relativa de reporte (IIR), rendimiento analítico ajustado (IRAR) y solidez de evidencia (ICE). Si ICE <50%, prevalece “Evidencia baja”. Con evidencia suficiente: baja intensidad + bajo rendimiento = “Prioridad de revisión”; alta intensidad + bajo rendimiento = “Revisar efectividad”; baja intensidad + alto rendimiento = “Seguimiento selectivo”; alta intensidad + alto rendimiento = “Monitoreo estable”; el resto = “Monitoreo ordinario”. Esta salida orienta revisión de reportabilidad; no equivale a la Prioridad de Fiscalización de ATLAS, que mantiene IRAR-E, IGR, IPA y mitigación como capas separadas y excluye IVO.'};
  }catch{}
  ...
  if(typeof v036Dashboard==='function'){
    ...
    v036Dashboard=function atlasIrarDashboard(ctx){
      ...
      html=html.replace('Los filtros gobiernan la matriz; cada sector abre un dossier explicativo en el mismo lugar.','Los filtros gobiernan la matriz; IIR, IRAR e ICE construyen una señal de revisión de reportabilidad. IRAR-E, IGR, IPA e IVO conservan sus dominios propios y no se mezclan en esta posición. Cada sector abre un dossier explicativo en el mismo lugar.');
      return html;
    };
  }

  function irarCell(row){
    if(!Number.isFinite(Number(row?.irarAdjusted)))return '<div class="v036-mxn strong atlas-irar" data-v036-tip="IRAR|Sin base|No existen ROS enviados en 2021–2025 o no hay correspondencia sectorial suficiente.">—</div>';
    const profile=row.reportabilityProfile?.label||'Sin base';
    const tip=`IRAR|${fmt(row.irarAdjusted,2)}% · ${profile}|Observado ${fmt(row.irarObserved,2)}% · pares ${fmt(row.irarPeer,2)}% · ICE ${fmt(row.ice,0)}% · ${row.irarFamily}`;
  ...
  function profileInterpretation(row){
    ...
    const map={
      ...
      MONITOREO_ESTABLE:'Con evidencia suficiente, el sector combina alta intensidad y alto rendimiento relativo. No surge una señal comparativa prioritaria en reportabilidad; corresponde mantener monitoreo y revisar cambios relevantes.',
      ...
      SIN_BASE:'No existe base suficiente para una lectura combinada IIR × IRAR × ICE.'
    };
  ...
  function renderProfileMap(){
    ...
    panel.innerHTML=`<div class="atlas-irar-map-head"><div><span>MAPA DE REPORTABILIDAD</span><h3>Intensidad, rendimiento y evidencia ${typeof v036Help==='function'?v036Help('reportability_profile'):''}</h3><p>Derecha = mayor intensidad relativa (IIR) · Arriba = mayor rendimiento relativo (IRAR). Tamaño y opacidad = ICE. La clasificación resultante es una señal de revisión de reportabilidad.</p>${legendHtml}<p class="atlas-irar-scope-note"><b>Arquitectura ATLAS:</b> este mapa usa sólo IIR × IRAR × ICE. IRAR-E (riesgo sectorial), IGR (territorio) e IPA (entidad) permanecen como capas separadas de la priorización de fiscalización; IVO queda fuera y se utiliza para screening de potenciales SO.</p></div><div class="atlas-irar-map-legend"><span>Prioridad de revisión <b>${counts.PRIORIDAD_REVISION}</b></span><span>Revisar efectividad <b>${counts.REVISAR_EFECTIVIDAD}</b></span><span>Seguimiento selectivo <b>${counts.SEGUIMIENTO_SELECTIVO}</b></span><span>Monitoreo estable <b>${counts.MONITOREO_ESTABLE}</b></span><span>Monitoreo ordinario <b>${counts.MONITOREO_ORDINARIO}</b></span><span>Evidencia baja <b>${counts.EVIDENCIA_BAJA}</b></span></div></div><div class="atlas-irar-plane"><span class="atlas-irar-zone-label">Zona esperada</span><span class="atlas-irar-q q1">Baja intensidad · alto rendimiento</span><span class="atlas-irar-q q2">Alta intensidad · alto rendimiento</span><span class="atlas-irar-q q3">Baja intensidad · bajo rendimiento</span><span class="atlas-irar-q q4">Alta intensidad · bajo rendimiento</span><span class="atlas-irar-axis-label xlow">IIR bajo</span><span class="atlas-irar-axis-label xhigh">IIR alto</span><span class="atlas-irar-axis-label ylow">IRAR relativo bajo</span><span class="atlas-irar-axis-label yhigh">IRAR relativo alto</span></div><div class="atlas-irar-map-foot"><b>Lectura supervisora:</b> la posición combina IIR e IRAR; ICE determina si existe evidencia suficiente para interpretarla. Con ICE &lt;50% prevalece “Evidencia baja”. Con ICE ≥50%, los extremos se traducen en señales de revisión o seguimiento. IRAR-E, IGR e IPA no alteran estas coordenadas, evitando doble conteo; IVO pertenece al carril de potenciales SO. Son señales comparativas, no hallazgos ni determinaciones de incumplimiento.</div>`;
    ...
    const dots=rows.map(r=>{
      ...
      const meta=segMeta(r.seg); const tip=`${r.name}|${r.reportabilityProfile?.label||'Sin perfil'}|IIR ${fmt(r.iir,2)}× · IRAR ${fmt(r.irarAdjusted,2)}% · relativo ${fmt(r.irarRelative,2)}× · ICE ${fmt(r.ice,0)}% · ${meta.label}`;
  ...
  window.ATLAS_IRAR_UI={version:core.version,integration_version:UI_VERSION,profile_map:'IIR_X_IRAR_X_ICE',primary:'IRAR_ADJUSTED',evidence:'ICE',classification:'REPORTABILITY_REVIEW_SIGNAL',min_evidence_ice:50,legacy:'ICR_OBSERVED_TRACEABILITY_ONLY'};
})();
