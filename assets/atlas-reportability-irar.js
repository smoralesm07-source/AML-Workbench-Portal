...
(function atlasReportabilityIrar(){
  ...
  function renderProfileMap(){
    ...
    const dots=rows.map(r=>{
      const idx=V036_STATE.rows.indexOf(r),x=pos(r.iir),y=100-pos(r.irarRelative),sent=Number(r.total)||0;
      const radius=Math.max(.9,Math.min(2.4,.9+.32*Math.log10(sent+1))),opacity=Math.max(.42,Math.min(1,(Number(r.irarCredibility)||0)/100));
      const color=V036_SEG?.[r.seg]?.color||'#5bb4f5';
      ...
      return `<circle class="atlas-irar-dot ${profileClass(r.reportabilityProfile?.key)}" cx="${x}" cy="${y}" r="${radius}" fill="${esc(color)}" fill-opacity="${opacity}" stroke="#e7eef5" stroke-opacity=".72" stroke-width=".22" vector-effect="non-scaling-stroke" data-atlas-irar-row="${idx}" data-v036-tip="${esc(tip)}" aria-label="${esc(r.name)}" role="button" tabindex="0"></circle>`;
    }).join('');
    const xLow=pos(.75),xHigh=pos(1.50),yLow=pos(.80),yHigh=pos(1.25),zoneTop=100-yHigh,zoneBottom=100-yLow;
    const zoneWidth=xHigh-xLow,zoneHeight=zoneBottom-zoneTop;
    const svg=`<svg class="atlas-irar-svg" viewBox="0 0 100 100" preserveAspectRatio="none" role="img" aria-label="Mapa IIR 2025 por IRAR relativo 2021–2025"><rect class="atlas-irar-expected-zone-svg" x="${xLow}" y="${zoneTop}" width="${zoneWidth}" height="${zoneHeight}" rx="1.2" ry="1.2"></rect><line class="atlas-irar-threshold-svg" x1="${xLow}" y1="0" x2="${xLow}" y2="100"></line><line class="atlas-irar-threshold-svg" x1="${xHigh}" y1="0" x2="${xHigh}" y2="100"></line><line class="atlas-irar-threshold-svg" x1="0" y1="${zoneTop}" x2="100" y2="${zoneTop}"></line><line class="atlas-irar-threshold-svg" x1="0" y1="${zoneBottom}" x2="100" y2="${zoneBottom}"></line><line class="atlas-irar-axis-svg" x1="50" y1="0" x2="50" y2="100"></line><line class="atlas-irar-axis-svg" x1="0" y1="50" x2="100" y2="50"></line>${dots}</svg>`;
    panel.innerHTML=`<div class="atlas-irar-map-head"><div><span>PERFIL DE REPORTABILIDAD</span><h3>IIR 2025 × IRAR ajustado 2021–2025</h3><p>Eje X: intensidad relativa de reporte 2025. Eje Y: rendimiento analítico histórico ajustado relativo a pares. Tamaño = ROS acumulados 5 años · opacidad = credibilidad.</p></div><div class="atlas-irar-map-legend"><span>Intensivo–productivo <b>${counts.INTENSIVO_PRODUCTIVO}</b></span><span>Intensivo–bajo rendimiento <b>${counts.INTENSIVO_BAJO_RENDIMIENTO}</b></span><span>Selectivo–productivo <b>${counts.SELECTIVO_PRODUCTIVO}</b></span><span>Baja activación <b>${counts.BAJA_ACTIVACION}</b></span><span>Esperado <b>${counts.COMPORTAMIENTO_ESPERADO}</b></span></div></div><div class="atlas-irar-plane">${svg}<span class="atlas-irar-zone-label">Zona esperada</span><span class="atlas-irar-q q1">Selectivo–productivo</span><span class="atlas-irar-q q2">Intensivo–productivo</span><span class="atlas-irar-q q3">Baja activación</span><span class="atlas-irar-q q4">Intensivo–bajo rendimiento</span><span class="atlas-irar-axis-label xlow">IIR bajo</span><span class="atlas-irar-axis-label xhigh">IIR alto</span><span class="atlas-irar-axis-label ylow">IRAR relativo bajo</span><span class="atlas-irar-axis-label yhigh">IRAR relativo alto</span></div><div class="atlas-irar-map-foot"><b>Zona esperada:</b> IIR entre 0,75× y 1,50× e IRAR relativo entre 0,80× y 1,25×. Los ejes finos marcan 1,0×; las líneas discontinuas marcan los umbrales de perfil. Los extremos son señales comparativas para revisión, no hallazgos ni determinaciones de cumplimiento.</div>`;
