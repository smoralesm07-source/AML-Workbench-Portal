...
(function atlasEntityIntelligence0520(){
  ...
  if(!BASE_RENDER){
    window.__ATLAS_ENTITY_INTELLIGENCE_0520__={active:false,reason:'entity-renderer-unavailable'};
    return;
  }
  ...
  function statusClass(v){
    return ({healthy:'ok',fresh:'ok',watch:'warn',stale:'warn',degraded:'bad',unknown:'neutral'})[String(v||'unknown')]||'neutral';
  }
  ...
  async function fetchPackage(entity){
    ...
    return {
      entityId:id,
      enrichment:Array.isArray(enrichment?.data)?enrichment.data:[],
      health:Array.isArray(health?.data)?health.data:[],
      errors:[enrichment?.error,health?.error].filter(Boolean).map(e=>String(e?.message||e)),
      loadedAt:Date.now()
    };
  }
  ...
  async function hydrate(pkg){
    ...
    const job=(async()=>{
      const data=await fetchPackage(pkg.e);
  ...
  function packageFor(entity){
    const hit=CACHE.get(entity?.entity_id);
  ...
  function sourceCard(row,signals){
    ...
    const confirmed=signals.filter(s=>['observed','confirmed_match'].includes(s.signal_status));
    const possible=signals.filter(s=>s.signal_status==='possible_match');
    const lastSignal=signals[0]?.refreshed_at||null;
    const lastIngest=row?.last_successful_ingest_at||null;
    return `<article class="aei-source-card">
      <div class="aei-source-head"><div><b>${esc(row?.source_name||signals[0]?.source_code||'Fuente')}</b><small>${esc(row?.source_class||'fuente externa')}</small></div><span class="aei-status ${status.cls}">${esc(status.label)}</span></div>
      <div class="aei-source-metrics">
        <span><b>${confirmed.length}</b><small>hallazgos</small></span>
        <span><b>${possible.length}</b><small>posibles match</small></span>
      </div>
      <p>${signals.length?esc(signals[0]?.summary||signals[0]?.title||'Existe evidencia materializada para esta entidad.'):'Sin hallazgos materializados para esta entidad.'}</p>
      <footer><span>Último dato: ${esc(day(row?.last_source_record_at||lastSignal))}</span><span>Última ingesta: ${esc(day(lastIngest))}</span></footer>
    </article>`;
  }
  ...
  function signalRows(signals){
    ...
    return `<div class="aei-findings">${signals.slice(0,12).map(s=>`<div class="aei-finding">
      <div><span class="aei-source-code">${esc(s.source_code)}</span><b>${esc(s.title||s.signal_type||'Hallazgo externo')}</b></div>
      <p>${esc(s.summary||'Sin resumen adicional.')}</p>
      <dl><dt>Estado</dt><dd>${esc(String(s.signal_status||'—').replaceAll('_',' '))}</dd><dt>Confianza</dt><dd>${s.match_confidence==null?'—':Math.round(Number(s.match_confidence)*100)+'%'}</dd><dt>Jurisdicción</dt><dd>${esc(s.jurisdiction||'—')}</dd><dt>Corte</dt><dd>${esc(day(s.refreshed_at))}</dd></dl>
    </div>`).join('')}</div>`;
  }

  function render(pkg,data){
    const healthByCode=new Map((data?.health||[]).map(r=>[r.source_code,r]));
    ...
    return `<section class="aei-card" id="aei-entity-intelligence">
      <header class="aei-main-head"><div><span class="aei-eyebrow">ENTITY INTELLIGENCE</span><h3>Inteligencia externa de la entidad</h3><p>Enriquecimiento separado del score IPA3. Una coincidencia, relación o sanción externa no transfiere riesgo ni constituye conclusión AML.</p></div><div class="aei-kpis"><span><b>${signals.length}</b><small>registros</small></span><span><b>${new Set(signals.map(s=>s.source_code)).size}</b><small>fuentes con datos</small></span></div></header>
      ${groups}
      <section class="aei-evidence"><header><h4>Evidencia materializada</h4><span>${esc(ENRICHMENT_TABLE)}</span></header>${signalRows(signals)}</section>
      ${data?.errors?.length?`<div class="aei-error">Bloques no disponibles: ${esc(data.errors.join(' · '))}</div>`:''}
    </section>`;
  }
  ...
  function paint(pkg,data){
    ...
    if(!root)return;
    const panel=root.querySelector('[data-a45-panel="evidence"]')||root.querySelector('[data-a45-panel="character"]');
    if(!panel)return;
    let holder=panel.querySelector('[data-aei-holder="entity-intelligence"]');
    if(!holder){holder=document.createElement('div');holder.dataset.aeiHolder='entity-intelligence';holder.style.marginTop='16px';panel.appendChild(holder);}
    holder.innerHTML=data?render(pkg,data):'<section class="aei-card"><div class="aei-loading">Consultando inteligencia externa gobernada…</div></section>';
  ...
  function wrappedRender(pkg,preserve=false){
    const result=BASE_RENDER(pkg,preserve);
    try{decorate(pkg);}catch(_error){}
    return result;
  }

  try{v0203RenderEntity=wrappedRender;}catch(_error){}
  window.v0203RenderEntity=wrappedRender;
