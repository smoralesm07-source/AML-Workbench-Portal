...
/* ATLAS AML 0.52.2 · Entity Intelligence
 * Capa transversal de enriquecimiento externo para Entity 360.
 * No modifica IPA3 ni transfiere riesgo entre entidades.
 * Mercado Público: OC = flujo monetario; licitación = contexto del proceso.
 */
(function atlasEntityIntelligence0522(){
  const RELEASE='0.52.2',BUILD='0522';
  ...
  function analyticsBlock(code,live){
    ...
    if(code==='MERCADO_PUBLICO'){
      ...
      return `<div class="aei-analytics"><span><b>${Number(a.orders||0)}</b><small>órdenes de compra</small></span><span><b>$${money(a.observed_purchase_amount)}</b><small>compra neta observada</small></span><span><b>${Number(a.tenders||0)}</b><small>licitaciones · contexto</small></span><span><b>${pct(a.top_organism_share_by_orders)}</b><small>concentración OC</small></span></div>${coverage.window_days?`<div class="aei-coverage">Cobertura live: ${esc(coverage.window_days)} días · OC = flujo monetario observado; licitación = participación/proceso. Esta ventana no representa historia completa ni gasto total del proveedor.</div>`:''}`;
    }
    if(code==='INFOLOBBY')return `<div class="aei-analytics"><span><b>${Number(a.audiences||0)}</b><small>audiencias únicas</small></span><span><b>${Number(a.participant_records||0)}</b><small>registros de participantes</small></span></div>`;
    if(code==='ATLAS_CROSS')return `<div class="aei-cross-summary"><b>${Number(a.overlaps||0)} coincidencia(s) OC × lobby</b><span>Coocurrencia por organismo para orientar revisión. No prueba causalidad, trato preferente ni irregularidad.</span></div>`;
    return '';
  }
  function sourceCard(code,row,signals,live){
    ...
    if(live?.status==='not_found')note='El RUT no fue resuelto como proveedor en la API oficial de Mercado Público.';
  ...
  function signalRows(signals){
    ...
    return `<div class="aei-findings">${signals.slice(0,22).map(s=>{const e=s.evidence||{};const extra=s.signal_type==='procurement_lobby_overlap'?`<div class="aei-overlap"><span>Órdenes de compra <b>${esc(e.mercado_publico_ordenes||0)}</b></span><span>InfoLobby <b>${esc(e.infolobby_audiencias||0)}</b></span><span>Monto observado <b>$${money(e.monto_compra_observado)}</b></span></div>`:'';return `<div class="aei-finding ${s.source_code==='ATLAS_CROSS'?'aei-finding-cross':''}"><div><span class="aei-source-code">${esc(s.source_code)}</span><b>${esc(s.title||s.signal_type||'Hallazgo externo')}</b></div><p>${esc(s.summary||'Sin resumen adicional.')}</p>${extra}<dl><dt>Estado</dt><dd>${esc(String(s.signal_status||'—').replaceAll('_',' '))}</dd><dt>Confianza</dt><dd>${s.match_confidence==null?'—':Math.round(Number(s.match_confidence)*100)+'%'}</dd><dt>Modo</dt><dd>${s._live?'live':'snapshot'}</dd><dt>Corte</dt><dd>${esc(day(s.refreshed_at))}</dd></dl></div>`;}).join('')}</div>`;
  }
  function render(data){
    ...
    return `<section class="aei-card" id="aei-entity-intelligence"><header class="aei-main-head"><div><span class="aei-eyebrow">ENTITY INTELLIGENCE</span><h3>Inteligencia externa de la entidad</h3><p>Consulta live autenticada y snapshots gobernados, separados del score IPA3. Mercado Público distingue órdenes de compra de licitaciones; los cruces son señales y no transfieren riesgo.</p></div><div class="aei-kpis"><span><b>${signals.length}</b><small>registros</small></span><span><b>${new Set(signals.map(s=>s.source_code)).size}</b><small>fuentes con datos</small></span><span><b>${cross.length}</b><small>cruces OC × lobby</small></span></div></header>${groups}<section class="aei-evidence"><header><h4>Evidencia disponible</h4><span>LIVE + ${esc(ENRICHMENT_TABLE)}</span></header>${signalRows(signals)}</section>${data?.errors?.length?`<div class="aei-error">Bloques no disponibles: ${esc(data.errors.join(' · '))}</div>`:''}</section>`;
  }
  function paint(pkg,data){const root=document.querySelector('#content .a45');if(!root)return;const panel=root.querySelector('[data-a45-panel="evidence"]')||root.querySelector('[data-a45-panel="character"]');if(!panel)return;let holder=panel.querySelector('[data-aei-holder="entity-intelligence"]');if(!holder){holder=document.createElement('div');holder.dataset.aeiHolder='entity-intelligence';holder.style.marginTop='16px';panel.appendChild(holder);}holder.innerHTML=data?render(data):'<section class="aei-card"><div class="aei-loading">Consultando Mercado Público, InfoLobby y evidencia gobernada…</div></section>';const mp=data?.live?.sources?.MERCADO_PUBLICO||null;window.__ATLAS_ENTITY_INTELLIGENCE_0520__={active:true,release:RELEASE,build:BUILD,entityId:pkg?.e?.entity_id||null,hydrated:!!data,liveMode:true,crossMode:'PURCHASE_ORDER_INFOLOBBY',procurementStatus:mp?.status||null,procurementCoverage:mp?.coverage||null,moneyFlowSemantics:'PURCHASE_ORDERS_PRIMARY',sources:SOURCE_GROUPS.flatMap(g=>g.codes),errors:data?.errors||[],renderedAt:new Date().toISOString()};}
  ...
  window.__ATLAS_ENTITY_INTELLIGENCE_0520__={active:true,release:RELEASE,build:BUILD,hydrated:false,liveMode:true,crossMode:'PURCHASE_ORDER_INFOLOBBY',cachePolicy:'MEMORY_ONLY',scoreMutation:false,installedAt:new Date().toISOString()};
})();