...
(function atlasEntityIntelligence0520(){
  ...
  function analyticsBlock(code,live){
    ...
    if(code==='MERCADO_PUBLICO'){
      ...
      return `<div class="aei-analytics"><span><b>${Number(a.tenders||0)}</b><small>registros ${coverage.window_days?coverage.window_days+' días':'recientes'}</small></span><span><b>${Number(a.awards_with_amount||0)}</b><small>con monto adjudicado</small></span><span><b>$${money(a.observed_award_amount)}</b><small>monto adjudicado observado</small></span><span><b>${pct(a.top_organism_share_by_records)}</b><small>concentración por registros</small></span></div>${coverage.window_days?`<div class="aei-coverage">Cobertura: ventana móvil ${esc(coverage.window_days)} días · detalle hasta ${esc(coverage.detail_cap||0)} licitaciones · no representa historia completa, gasto total ni participación total del proveedor.</div>`:''}`;
    }
