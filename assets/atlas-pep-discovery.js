...
function linkageLabel(v){return({DECLARED_INTEREST:'Interés societario declarado',BF_CALCULATED_DIRECT:'BF directo',BF_CALCULATED_INDIRECT:'BF indirecto',CONTROL_DECLARED:'Control declarado'})[v]||String(v||'Vínculo');}
function signalLabel(v){return v==='DECLARED_INTEREST'?'Interés declarado':String(v||'');}
function isBfLink(c){return ['BF_CALCULATED_DIRECT','BF_CALCULATED_INDIRECT'].includes(String(c?.linkage_strength||''));}
...
function flow(p){const c=coverage(p);return`<div class="atlas-pep-flow" aria-label="Cadena analítica PEP, intereses declarados y compras">
  ...
  <div class="atlas-pep-flow-step"><small>2 · Intereses</small><b>${fmt(c.declared_interest_company_ruts)}</b><span>RUT societarios declarados vigentes</span></div>
  <div class="atlas-pep-flow-step"><small>3 · Compras</small><b>${fmt(c.declared_interest_provider_matches)}</b><span>sociedades declaradas que aparecen como proveedor</span></div>
...
function kpis(p){const c=coverage(p);return`<div class="atlas-pep-kpis">
  ...
  <article class="atlas-pep-kpi"><span class="lab">Interés declarado</span><b>${fmt(c.declared_interest_company_ruts)}</b><small>RUT de sociedad; no implica control ni BF</small></article>
  <article class="atlas-pep-kpi"><span class="lab">Interés + proveedor</span><b>${fmt(c.declared_interest_provider_matches)}</b><small>cruce exacto por RUT con compras públicas</small></article>
  <article class="atlas-pep-kpi"><span class="lab">BF downstream</span><b>${fmt(bfCount(p,'downstream_target_count'))}</b><small>sólo cuando existe % explícito gobernado</small></article>
...
function mix(p){
  const rows=topCases(p),direct=rows.filter(x=>x.linkage_strength==='BF_CALCULATED_DIRECT').length,indirect=rows.filter(x=>x.linkage_strength==='BF_CALCULATED_INDIRECT').length,control=rows.filter(x=>x.linkage_strength==='CONTROL_DECLARED').length,interest=rows.filter(x=>x.linkage_strength==='DECLARED_INTEREST').length,total=Math.max(1,direct+indirect+control+interest),p1=direct/total*100,p2=(direct+indirect)/total*100,p3=(direct+indirect+control)/total*100;
  const bg=`conic-gradient(var(--pep-accent) 0 ${p1.toFixed(2)}%,#7e77cf ${p1.toFixed(2)}% ${p2.toFixed(2)}%,var(--pep-muted) ${p2.toFixed(2)}% ${p3.toFixed(2)}%,#4fa3d1 ${p3.toFixed(2)}% 100%)`;
  return`<div class="atlas-pep-donut-wrap"><div class="atlas-pep-donut" style="background:${bg}"><div class="atlas-pep-donut-center"><b>${fmt(rows.length)}</b><span>casos top</span></div></div><div class="atlas-pep-legend">
    ...
    <div class="atlas-pep-legend-row"><i style="--c:#4fa3d1"></i><span>Interés societario declarado</span><b>${fmt(interest)}</b></div>
  </div></div>`;
}
...
function coveragePanel(p){
  ...
  return`<div class="atlas-pep-bars">
    ...
    <div class="atlas-pep-bar"><span class="atlas-pep-bar-label">RUT con interés declarado</span><div class="atlas-pep-track"><div class="atlas-pep-fill" style="--w:${Math.min(100,num(c.declared_interest_company_ruts)?100:0)}%"></div></div><span class="atlas-pep-bar-value">${fmt(c.declared_interest_company_ruts)}</span></div>
    <div class="atlas-pep-bar"><span class="atlas-pep-bar-label">Interés + proveedor</span><div class="atlas-pep-track"><div class="atlas-pep-fill" style="--w:${Math.min(100,num(c.declared_interest_provider_matches)?100:0)}%"></div></div><span class="atlas-pep-bar-value">${fmt(c.declared_interest_provider_matches)}</span></div>
    <div class="atlas-pep-bar"><span class="atlas-pep-bar-label">Órdenes coincidentes</span><div class="atlas-pep-track"><div class="atlas-pep-fill" style="--w:${Math.min(100,num(c.orders_matched)?100:0)}%"></div></div><span class="atlas-pep-bar-value">${fmt(c.orders_matched)}</span></div>
...
function filters(rows){const links=[...new Set(rows.map(x=>x.linkage_strength).filter(Boolean))],signals=[...new Set(rows.flatMap(x=>[...arr(x.relationship_signals),...arr(x.review_signals)]))].sort();return`<div class="atlas-pep-toolbar" aria-label="Filtros de descubrimiento">
  ...
  <select id="atlas-pep-signal" class="atlas-pep-field"><option value="ALL">Todas las señales</option>${signals.map(v=>`<option value="${esc(v)}" ${ui.signal===v?'selected':''}>${esc(signalLabel(v))}</option>`).join('')}</select>
</div>`;}
...
function relationEvidence(c){
  if(isBfLink(c))return`<div><div class="atlas-pep-kicker">Camino de propiedad económica</div>${pathHtml(c.beneficial_owner_paths)}</div>`;
  if(c.linkage_strength==='DECLARED_INTEREST')return'<div><div class="atlas-pep-kicker">Base del vínculo</div><div class="atlas-pep-method"><strong>Interés societario declarado.</strong> La declaración pública vincula a la persona con esta sociedad. Este vínculo no acredita porcentaje de propiedad, control ni beneficiario final.</div></div>';
  return'<div><div class="atlas-pep-kicker">Base del vínculo</div><div class="atlas-pep-method">El contrato informa un vínculo de control declarado gobernado; su lectura debe mantenerse separada del cálculo económico de beneficiario final.</div></div>';
}
function openDrawer(c){const economic=isBfLink(c)?(c.beneficial_owner_pct==null?'No calculado':esc(pct(c.beneficial_owner_pct))):(c.linkage_strength==='DECLARED_INTEREST'?'No acreditado':'No calculado');const html=`<div class="atlas-pep atlas-pep-drawer"><div><div class="atlas-pep-kicker">Relación trazable</div><h2>${esc(c.company_name||c.company_rut||'Entidad')}</h2><div class="atlas-pep-drawer-sub">${esc(c.company_rut||'')} · ${esc(c.person_name||'Persona')} · ${esc(c.position||'cargo no informado')} · ${esc(c.institution||'institución no informada')}</div></div><div class="atlas-pep-tags"><span class="atlas-pep-tag rel">${esc(linkageLabel(c.linkage_strength))}</span>${arr(c.relationship_signals).map(s=>`<span class="atlas-pep-tag rel">${esc(signalLabel(s))}</span>`).join('')}${arr(c.review_signals).map(s=>`<span class="atlas-pep-tag rev">${esc(s)}</span>`).join('')}</div><div class="atlas-pep-drawer-kpis"><div class="atlas-pep-drawer-kpi"><span>Beneficio económico</span><b>${economic}</b></div><div class="atlas-pep-drawer-kpi"><span>Nivel de lectura</span><b>${c.review_level==='REVIEW'?'Revisar':'Contexto'}</b></div><div class="atlas-pep-drawer-kpi"><span>Órdenes</span><b>${fmt(c.order_count)} · ${esc(clp(c.amount_total_clp))}</b></div><div class="atlas-pep-drawer-kpi"><span>Compradores</span><b>${fmt(c.distinct_buyers)} · HHI ${fmt(c.buyer_amount_hhi,2)}</b></div></div>${relationEvidence(c)}<div class="atlas-pep-method"><strong>Cómo leer este caso.</strong><br>${esc(c.guardrail||'La relación PEP/sociedad es contexto de debida diligencia y no una conclusión de irregularidad.')}<br><br><strong>Prioridad de compras:</strong> ${fmt(c.procurement_priority_score,1)} / 100 — triage, no score AML.</div></div>`;if(typeof window.v019OpenDrawer==='function')window.v019OpenDrawer(html);else{const body=document.querySelector('#v019-drawer-body'),drawer=document.querySelector('#v019-drawer');if(body&&drawer){body.innerHTML=html;drawer.classList.add('open');}}}
...
function render(p){
  ...
  host.innerHTML=`<section class="atlas-pep" data-atlas-pep-root="1"><div class="atlas-pep-hero"><div class="atlas-pep-hero-row"><div><div class="atlas-pep-kicker">Análisis · Personas, sociedades y Estado</div><h2>Descubrimiento PEP y relaciones societarias</h2><p>Distingue explícitamente tres niveles de evidencia: interés societario declarado, control gobernado y beneficiario final calculado. Cruza sólo RUT exactos con compras públicas y prioriza revisión humana sin convertir la condición PEP en score AML.</p></div><div class="atlas-pep-status"><span class="atlas-pep-pill ${statusClass(p)}">${esc(statusLabel(p))}</span>${failed?`<span class="atlas-pep-pill review">${fmt(failed)} período${failed===1?'':'s'} con incidencia</span>`:''}<span class="atlas-pep-pill">${esc(generated(p))}</span></div></div>${flow(p)}</div>${kpis(p)}<div class="atlas-pep-grid"><article class="atlas-pep-panel"><div class="atlas-pep-panel-head"><div><h3>Estructura de los casos top visibles</h3><p>Interés declarado, control y BF se mantienen semánticamente separados.</p></div></div>${mix(p)}</article><article class="atlas-pep-panel"><div class="atlas-pep-panel-head"><div><h3>Cobertura gobernada</h3><p>Se muestra únicamente lo informado por el contrato IFL; missing nunca se completa como cero.</p></div></div>${coveragePanel(p)}</article></div><article class="atlas-pep-panel"><div class="atlas-pep-panel-head"><div><h3>Materialidad de los casos top</h3><p>Monto de órdenes comprometidas; no representa pago efectivo.</p></div></div>${topBars(p)}</article>${filters(topCases(p))}<div id="atlas-pep-case-results">${caseRows(p)}</div><div class="atlas-pep-note"><strong>Regla de lectura.</strong> Interés declarado, PEP-01, PEP-02 y PEP-05 son contexto. Sólo PEP-03/PEP-04 llevan un caso a <em>Revisar</em>. El ranking es triage analítico, no probabilidad de delito ni score LA/FT.</div></section>`;
  bind(p);setNavActive();
}
...
async function open(force=false){
  ...
  try{if(typeof window.shell==='function')window.shell('Personas y control','PEP, intereses societarios, beneficiario final y compras públicas con trazabilidad y guardrails explícitos.');}catch(_e){}
