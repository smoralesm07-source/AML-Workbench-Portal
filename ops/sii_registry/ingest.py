#!/usr/bin/env python3
"""ATLAS AML · padrón público SII completo y screening SO por RUT exacto."""
...
AUD="atlas-sii-registry-ingest"; UA="Atlas-SII-Registry/0.65 (+official SII open data)"
...
def upper_norm(s):return re.sub(r"\s+"," ",unicodedata.normalize("NFKD",str(s or "")).encode("ascii","ignore").decode().upper()).strip()
def iso_date(v):
    s=str(v or "").strip()
...
def rut_valid(body,dv):
    ...
    total=0;factor=2
    for ch in reversed(b):total+=int(ch)*factor;factor=2 if factor==7 else factor+1
    r=11-(total%11);expected="0" if r==11 else "K" if r==10 else str(r)
    return f"{int(b)}-{d}" if d==expected else None
def pick(row,*names):
    for name in names:
...
def download(url,path):
    ...
    for attempt in range(3):
        try:
            req=urllib.request.Request(url,headers={"User-Agent":UA,"Accept":"*/*"});sha=hashlib.sha256();size=0
...
def open_zip_text(path,preferred):
    z=zipfile.ZipFile(path);member=next((n for n in z.namelist() if Path(n).name.upper()==preferred.upper()),None)
    if not member:raise RuntimeError(f"{preferred} missing")
    raw=z.open(member);sample=raw.read(120000);raw.close();enc="utf-8-sig"
    try:sample.decode(enc)
    except UnicodeDecodeError:enc="cp1252"
    stream=io.TextIOWrapper(z.open(member),encoding=enc,errors="replace",newline="");first=stream.read(12000);stream.seek(0)
...
def canonical_reader(stream,dialect):
    reader=csv.DictReader(stream,dialect=dialect);fields=[norm(x) for x in (reader.fieldnames or [])]
    for raw in reader:yield {fields[i]:v for i,v in enumerate(raw.values()) if i<len(fields)}
...
def snapshot(kind,url,sha,size,headers):
    sid=f"sii:{kind}:{sha}";status=api({"action":"snapshot_status","source_kind":kind,"source_hash":sha})
    if status.get("exists") and (status.get("snapshot") or {}).get("status")=="NORMALIZED":return sid,True,status.get("snapshot") or {}
...
def load_policy(path):
    raw=path.read_bytes();sha=hashlib.sha256(raw).hexdigest();sid=f"sii:POLICY:{sha}";status=api({"action":"snapshot_status","source_kind":"POLICY","source_hash":sha});eligible=set();rows=[]
    for r in csv.DictReader(io.StringIO(raw.decode("utf-8-sig",errors="replace")),delimiter=";"):
    ...
    if not (status.get("exists") and (status.get("snapshot") or {}).get("status")=="NORMALIZED"):
        ...
        api({"action":"finalize","source_kind":"POLICY","snapshot_id":sid,"source_hash":sha,"record_count":len(rows),"accepted_count":len(rows),"metadata":{"eligible_candidate_use_si_codes":len(eligible)}})
    return eligible,sha
def load_names(path,sha,size,headers):
    sid,skip,old=snapshot("NAMES",NAMES_URL,sha,size,headers)
    if skip:return {"skipped":True,"snapshot_id":sid,"observed":old.get("record_count"),"accepted":old.get("accepted_count")}
    z,stream,dialect=open_zip_text(path,"PUB_NOMBRES_PJ.txt");observed=accepted=0
    def rows():
        nonlocal observed,accepted
        for r in canonical_reader(stream,dialect):
            observed+=1;rut=rut_valid(pick(r,"rut","rut_contribuyente"),pick(r,"dv","digito_verificador"));name=str(pick(r,"razon_social","nombre_razon_social","nombre") or "").strip()
    ...
    api({"action":"finalize","source_kind":"NAMES","snapshot_id":sid,"source_hash":sha,"record_count":observed,"accepted_count":accepted,"metadata":{"coverage":"ALL_VALID_LEGAL_ENTITY_TAXPAYERS","official":"SII","last_modified":headers.get("Last-Modified")}})
    return {"snapshot_id":sid,"observed":observed,"accepted":accepted}
def load_activities(path,sha,size,headers,eligible):
    sid,skip,old=snapshot("ACTIVITIES",ACT_URL,sha,size,headers)
    if skip:
        fin=api({"action":"finalize","source_kind":"ACTIVITIES","snapshot_id":sid,"source_hash":sha,"record_count":old.get("record_count"),"accepted_count":old.get("accepted_count"),"metadata":{"filtered_to_candidate_use_si":True,"eligible_codes":len(eligible),"official":"SII","last_modified":headers.get("Last-Modified")}})
        return {"skipped":True,"snapshot_id":sid,"potential":fin.get("potential")}
    z,stream,dialect=open_zip_text(path,"PUB_NOM_ACTECOS.txt");observed=accepted=0
    def rows():
        nonlocal observed,accepted
        for r in canonical_reader(stream,dialect):
            ...
            if not rut:continue
            name=str(pick(r,"desc_actividad_economica","actividad_economica","glosa_actividad","descripcion_actividad","actividad") or "").strip();date=iso_date(pick(r,"fecha","fecha_actividad","fecha_inscripcion"));accepted+=1
    ...
    fin=api({"action":"finalize","source_kind":"ACTIVITIES","snapshot_id":sid,"source_hash":sha,"record_count":observed,"accepted_count":accepted,"metadata":{"filtered_to_candidate_use_si":True,"eligible_codes":len(eligible),"official":"SII","last_modified":headers.get("Last-Modified")}})
...
def run():
    with tempfile.TemporaryDirectory() as td:
        ...
        print(json.dumps({"ok":True,"policy_hash":psha,"candidate_use_si_codes":len(eligible),"names":n,"activities":a},ensure_ascii=False))
def self_test():
    assert rut_valid("76086428","5")=="76086428-5";assert rut_valid("76086428","0") is None
    assert iso_date("31/12/2025")=="2025-12-31";assert upper_norm("Gestión Inmobiliária")=="GESTION INMOBILIARIA"
    print("SII registry ingest self-test OK")
if __name__=="__main__":
    ap=argparse.ArgumentParser();ap.add_argument("--self-test",action="store_true");args=ap.parse_args();self_test() if args.self_test else run()
