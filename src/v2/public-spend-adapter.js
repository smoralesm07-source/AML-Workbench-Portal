...
(function installAtlasV2PublicSpendAdapter(global) {
  ...
  function injectStyle() {
    ...
    style.textContent = `
      .gpv2{display:grid;gap:14px;color:var(--text,#18222b)}.gpv2 *{box-sizing:border-box}.gpv2 button,.gpv2 input,.gpv2 select{font:inherit}
      ...
      .gpv2-filters{display:grid;grid-template-columns:1fr 1.35fr 1fr auto;gap:10px;align-items:end;padding:14px;border:1px solid #dfe5e9;border-radius:14px;background:#fff}.gpv2-filter label{display:block;font-size:11px;font-weight:750;color:#65717a;margin:0 0 5px}.gpv2-filter select{width:100%;border:1px solid #ccd5db;border-radius:10px;padding:9px 10px;background:#fff;color:#26343d}
      ...
      .gpv2-empty,.gpv2-loading,.gpv2-error{padding:26px;text-align:center;border:1px dashed #ccd5db;border-radius:14px;background:#fff}.gpv2-error{color:#8a2f2f}.gpv2-pager{display:flex;justify-content:space-between;align-items:center;margin-top:10px}.gpv2-pager button{border:1px solid #ccd5db;background:#fff;border-radius:9px;padding:7px 10px;cursor:pointer}.gpv2-pager button:disabled{opacity:.45;cursor:default}
      .gpv2-trend{height:190px;display:flex;align-items:flex-end;gap:8px;padding:12px 4px 0;border-top:1px solid #eef1f3}.gpv2-trend-col{flex:1;min-width:0;height:100%;display:flex;flex-direction:column;justify-content:flex-end;align-items:center;gap:5px}.gpv2-barbox{height:145px;width:100%;display:flex;align-items:flex-end;justify-content:center}.gpv2-bar{width:min(24px,72%);min-height:2px;border-radius:7px 7px 2px 2px;background:#c76b26}.gpv2-trend-col span,.gpv2-trend-col b{font-size:9px;white-space:nowrap}.gpv2-trend-col span{color:#73808a}.gpv2-trend-col b{font-weight:650;color:#45535d}
      .gpv2-relation{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:10px;border-top:1px solid #eef1f3;padding:9px 2px}.gpv2-relation:first-child{border-top:0}.gpv2-relation small{display:block;color:#68747d}.gpv2-relation strong{white-space:nowrap;font-size:12px}.gpv2-relation-actions{display:flex;justify-content:flex-end;margin-top:5px}.gpv2-relation-actions button{border:1px solid #d9e0e4;background:#fff;border-radius:8px;padding:4px 7px;font-size:10px;cursor:pointer}
      ...
      @media(max-width:1000px){.gpv2-filters{grid-template-columns:1fr 1fr}.gpv2-filter-actions{align-self:end}}@media(max-width:900px){.gpv2-kpis{grid-template-columns:1fr 1fr}.gpv2-grid,.gpv2-method{grid-template-columns:1fr}.gpv2-hero{display:block}.gpv2-health{margin-top:12px}.gpv2-toolbar{align-items:stretch}.gpv2-toolbar input{flex:1}}@media(max-width:620px){.gpv2-filters{grid-template-columns:1fr}.gpv2-kpis{grid-template-columns:1fr}.gpv2-trend{gap:3px}.gpv2-trend-col b{display:none}}
    `;
  ...
  function trendChart(rows) {
    ...
    return `<div class="gpv2-trend">${data.map(row => { const value = num(row.amount_clp), height = Math.max(2, Math.round((value / max) * 100)), shortMonth = String(row.period || '').slice(5); return `<div class="gpv2-trend-col" title="${esc(row.period)} · ${esc(money(value))}"><div class="gpv2-barbox"><div class="gpv2-bar" style="height:${height}%"></div></div><b>${esc(money(value))}</b><span>${esc(shortMonth)}</span></div>`; }).join('')}</div>`;
  }
