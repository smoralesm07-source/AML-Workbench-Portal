...
// This contract now protects the explicit legacy rollback surface. The v2
// analytics shell is the product route authority, so the obsolete final-module
// routePinned flag is no longer a required product invariant.
...
console.log(`ATLAS legacy rollback Entity 360 + federated unreconciled press discovery + SII document authorization contract OK under release ${release.release}/${release.build}`);
