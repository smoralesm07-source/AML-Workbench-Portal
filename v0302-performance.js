...
const V0302_PIPELINES=['AML_MAIN','SII_ENTITY_YEAR','OSFL_PROFILE','SANCTION_IDENTITY'];
...
/* OSFL opens with one compact row (~all charts/KPIs) and pages entities from its runtime table. */
...
async function v0302PipelineHealth(force=false){
  ...
  V0302_HEALTH_PROMISE=(async()=>{const {data,error}=await sb.from('aml_sync_state').select('pipeline,status,updated_at,detail').in('pipeline',V0302_PIPELINES);if(error)throw error;return new Map((data||[]).map(r=>[r.pipeline,r]));})();
...
v024AuditHtml=function(f){
  ...
  const h=f.pipelineHealth||{},rows=[['Fusion',h.main],['SII',h.sii],['OSFL',h.osfl],['Sanciones',h.san]];const bad=rows.some(([,r])=>!r||String(r.status).toUpperCase()!=='SUCCESS');
  ...
  return `<section class="v024-audit ${bad?'pending':'ok'}"><button type="button" class="v024-audit-summary" data-v024-audit-toggle aria-expanded="false"><span class="v024-audit-title">${v024StatusDot(bad?'pending':'ok','Auditoría de datos')}</span><span class="v024-audit-lights">${rows.map(([label,r])=>v024StatusDot(v0302PipeStatus(r).cls,label)).join('')}</span><b>${bad?'Revisar pipeline':'Operativa'}</b><small>clic para detalle</small></button><div class="v024-audit-detail" data-v024-audit-detail hidden>${rows.map(([label,r])=>`<div><span>${esc(label)}</span><b>${esc(v0302PipeStatus(r).label)}</b><small>${esc(v0302AgeLabel(r?.updated_at))}${r?.detail?.rows?` · ${v019Fmt(r.detail.rows)} filas`:''}</small></div>`).join('')}${context.length?`<p><b>Contexto no bloqueante:</b> ${esc(context.join(' · '))}. La ausencia contextual no invalida los datos estructurados.</p>`:'<p>Todos los sellos estructurados disponibles están en estado SUCCESS.</p>'}</div></section>`;
};
...
window.AML_RUNTIME_SNAPSHOTS={version:V0302,overview:V0302_OVERVIEW,osfl:V0302_OSFL_DASH,ipa3:V0302_IPA_SNAPSHOT};
