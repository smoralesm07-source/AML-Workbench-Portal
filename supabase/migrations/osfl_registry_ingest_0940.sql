...
-- Safe full-population ingestion: source audit -> staging -> quality gate -> atomic promotion -> identity reconciliation.
...
create index if not exists aml_osfl_registry_stage_load_registry_idx
  on public.aml_osfl_registry_stage(load_id, registry_number);
create index if not exists aml_osfl_registry_stage_load_active_idx
  on public.aml_osfl_registry_stage(load_id, is_active);
create index if not exists aml_osfl_registry_ingest_run_snapshot_idx
  on public.aml_osfl_registry_ingest_run(snapshot_date desc, started_at desc);
create index if not exists aml_osfl_registry_master_valid_rut_0940_idx
  on public.aml_osfl_registry_master(rut_is_valid, rut);
...
-- These are ingestion-control tables. They intentionally have no anon/authenticated DML policies.
...
create or replace function public.aml_finalize_osfl_registry_load_0940(
  p_load_id text,
  p_expected_active_total integer default null
)
returns jsonb
language plpgsql
security invoker
...
begin
  ...
  if v_run.status not in ('RECEIVING','STAGED','VALIDATING') then
    raise exception 'OSFL load % has invalid status %', p_load_id, v_run.status;
  end if;
  ...
  select coalesce(sum(n-1),0)::integer into v_duplicates
  from (
    select registry_number,count(*) n
  ...
  if v_ref is not null and v_ref > 0 then
    v_diff := abs(v_active-v_ref)::numeric / v_ref;
  end if;

  -- Hard quality gate. The national file must be recognizably complete before it can replace the master.
  if v_observed < 250000 then
    raise exception 'OSFL source too small for national promotion: % rows', v_observed;
  end if;
  if v_active < 200000 then
    raise exception 'OSFL active population too small for national promotion: %', v_active;
  end if;
  if v_accepted::numeric/nullif(v_observed,0) < 0.98 then
    raise exception 'OSFL accepted row ratio below 98%%: %/%', v_accepted, v_observed;
  end if;
  if v_duplicates > greatest(10, floor(v_accepted*0.0005)::integer) then
    raise exception 'OSFL duplicate registry numbers exceed tolerance: %', v_duplicates;
  end if;
  if v_ref is not null and v_diff > 0.08 then
    raise exception 'OSFL active count differs more than 8%% from expected: active %, expected %', v_active, v_ref;
  end if;

  -- Replace the legal master only after every quality test has passed, inside this transaction.
  ...
  insert into public.aml_osfl_registry_master(
    registry_number,legal_name,rut,rut_raw,rut_is_valid,origin,commune,region,address,
    organization_type,classification,grant_date,registration_date,legal_status,is_active,
    source_snapshot_date,source_file_name,source_record_hash,source_sheet,source_row_number,source_sha256,
  ...
  select distinct on (registry_number)
    ...
    nullif(btrim(organization_type),''),nullif(btrim(classification),''),grant_date,registration_date,
    nullif(btrim(legal_status),''),is_active,v_run.snapshot_date,v_run.source_file_name,source_record_hash,
    source_sheet,source_row_number,v_run.source_sha256,now(),now()
  from public.aml_osfl_registry_stage
  ...
  -- Tier 1: exact, checksum-valid RUT. Registro Civil raw RUT is retained separately for audit.
  ...
  -- Tier 2a: exact normalized name + commune, only when candidate is unique.
  with c as (
    select m.registry_number,o.entity_id,o.rut,
      count(*) over(partition by m.registry_number) as candidate_n
    from public.aml_osfl_registry_master m
  ...
  select registry_number,entity_id,rut,'MATCH_HIGH','NAME_EXACT_COMMUNE_EXACT',0.9700,'AUTO',
    jsonb_build_object('name_exact',true,'commune_exact',true),now()
  from c where candidate_n=1
  on conflict (registry_number,entity_id) do nothing;
  get diagnostics v_high = row_count;

  -- Tier 2b: exact normalized name + region, unique candidate only, for rows still unmatched.
  with c as (
    select m.registry_number,o.entity_id,o.rut,
      count(*) over(partition by m.registry_number) as candidate_n
    from public.aml_osfl_registry_master m
  ...
  select registry_number,entity_id,rut,'MATCH_HIGH','NAME_EXACT_REGION_EXACT',0.9400,'AUTO',
    jsonb_build_object('name_exact',true,'region_exact',true),now()
  from c where candidate_n=1
  on conflict (registry_number,entity_id) do nothing;
  get diagnostics v_high = v_high + row_count;

  insert into public.aml_osfl_registry_source_snapshot(
    snapshot_date,official_active_total,source_name,source_url,evidence_url,ingestion_status,ingested_row_count,
    notes,source_sha256,source_file_name,source_bytes,last_successful_ingest_at,automation_status,created_at,updated_at
  ) values (
    v_run.snapshot_date,v_active,
    ...
    ingested_row_count=excluded.ingested_row_count,
    notes=excluded.notes,
    source_sha256=excluded.source_sha256,
    source_file_name=excluded.source_file_name,
    source_bytes=excluded.source_bytes,
    last_successful_ingest_at=now(),automation_status='FULL_FILE_VALIDATED',updated_at=now();
  ...
  update public.aml_osfl_registry_ingest_run set
    ...
    quality=jsonb_build_object(
      'accepted_ratio',round(v_accepted::numeric/nullif(v_observed,0),6),
      'active_ratio',round(v_active::numeric/nullif(v_accepted,0),6),
      'rut_valid_ratio',round(v_valid_rut::numeric/nullif(v_with_rut,0),6),
      'missing_names',v_missing_names,
      'exact_links',v_exact,'high_links',v_high,
      'unlinked',v_accepted-v_exact-v_high
    ),
    finished_at=now(),updated_at=now(),error=null
  where load_id=p_load_id;

  insert into public.aml_external_source_health(
    source_code,source_name,source_class,integration_mode,authoritative_source,enabled,software_status,data_status,
    last_source_record_at,last_successful_ingest_at,last_check_at,notes,metadata,refreshed_at
  ) values (
    'REGISTRO_CIVIL_OSFL','Registro Civil · RNPJSFL','official_list','scheduled',
    'Servicio de Registro Civil e Identificación',true,'healthy','fresh',v_run.snapshot_date::timestamptz,now(),now(),
    ...
    software_status='healthy',data_status='fresh',last_source_record_at=excluded.last_source_record_at,
    last_successful_ingest_at=now(),last_check_at=now(),notes=excluded.notes,metadata=excluded.metadata,refreshed_at=now();

  return jsonb_build_object(
    'ok',true,'load_id',p_load_id,'observed_rows',v_observed,'accepted_rows',v_accepted,
    'active_rows',v_active,'inactive_rows',v_inactive,'rows_with_rut',v_with_rut,'valid_ruts',v_valid_rut,
    'duplicates',v_duplicates,'exact_links',v_exact,'high_links',v_high,'unlinked',v_accepted-v_exact-v_high
  );
exception when others then
  update public.aml_osfl_registry_ingest_run set status='FAILED',error=left(sqlerrm,2000),finished_at=now(),updated_at=now()
  where load_id=p_load_id;
  raise;
end
$$;

-- Candidate-only fuzzy matcher. It never consolidates identity: all results require review.
create or replace function public.aml_generate_osfl_registry_probable_links_0940(p_limit integer default 500)
returns jsonb
language plpgsql
security invoker
set search_path=public
...
begin
  ...
  with unresolved as (
    ...
    select u.registry_number,o.entity_id,o.rut,
      similarity(public.aml_norm_osfl_registry_text_0940(u.legal_name),public.aml_norm_osfl_registry_text_0940(o.name)) as sim,
      row_number() over(partition by u.registry_number order by similarity(public.aml_norm_osfl_registry_text_0940(u.legal_name),public.aml_norm_osfl_registry_text_0940(o.name)) desc,o.entity_id) rn,
      lead(similarity(public.aml_norm_osfl_registry_text_0940(u.legal_name),public.aml_norm_osfl_registry_text_0940(o.name))) over(partition by u.registry_number order by similarity(public.aml_norm_osfl_registry_text_0940(u.legal_name),public.aml_norm_osfl_registry_text_0940(o.name)) desc,o.entity_id) next_sim,
      u.legal_name registry_name,o.name entity_name,u.region registry_region,o.region entity_region,u.commune registry_commune,o.commune entity_commune
    from unresolved u
    join public.aml_osfl_entity_runtime_snapshot o
      on (
  ...
  from candidates
  where rn=1 and sim>=0.86 and (next_sim is null or sim-next_sim>=0.04)
  on conflict(registry_number,entity_id) do nothing;
  get diagnostics v_inserted=row_count;
  return jsonb_build_object('ok',true,'review_candidates_created',v_inserted,'review_status','PENDING_REVIEW');
end
$$;

revoke all on function public.aml_finalize_osfl_registry_load_0940(text,integer) from public,anon,authenticated;
revoke all on function public.aml_generate_osfl_registry_probable_links_0940(integer) from public,anon,authenticated;
grant execute on function public.aml_finalize_osfl_registry_load_0940(text,integer) to service_role;
grant execute on function public.aml_generate_osfl_registry_probable_links_0940(integer) to service_role;

create or replace view public.aml_v_osfl_registry_ingest_status_current
with (security_invoker=true)
as
with r as (
  select * from public.aml_osfl_registry_ingest_run order by started_at desc limit 1
), s as (
  select * from public.aml_osfl_registry_source_snapshot order by snapshot_date desc limit 1
), m as (
  select count(*)::bigint master_rows,
    count(*) filter(where coalesce(is_active,false))::bigint master_active,
    count(*) filter(where rut_is_valid is true)::bigint master_valid_rut
  from public.aml_osfl_registry_master
), l as (
  select count(*) filter(where match_status='MATCH_EXACT')::bigint exact_links,
    count(*) filter(where match_status='MATCH_HIGH')::bigint high_links,
    count(*) filter(where match_status='MATCH_PROBABLE')::bigint probable_links,
    count(*) filter(where review_status='PENDING_REVIEW')::bigint pending_review
  from public.aml_osfl_registry_entity_link
)
select
  s.snapshot_date,s.ingestion_status,s.official_active_total,s.ingested_row_count,s.source_file_name,s.source_sha256,
  s.last_successful_ingest_at,s.automation_status,
  r.load_id last_load_id,r.status last_load_status,r.started_at last_load_started_at,r.finished_at last_load_finished_at,
  r.error last_load_error,r.quality last_load_quality,
  m.master_rows,m.master_active,m.master_valid_rut,l.exact_links,l.high_links,l.probable_links,l.pending_review,
  now() refreshed_at
...
insert into public.aml_external_source_health(
  ...
  source_name=excluded.source_name,source_class=excluded.source_class,integration_mode=excluded.integration_mode,
  authoritative_source=excluded.authoritative_source,enabled=true,notes=excluded.notes,
  metadata=coalesce(public.aml_external_source_health.metadata,'{}'::jsonb)||excluded.metadata,last_check_at=now(),refreshed_at=now();

comment on table public.aml_osfl_registry_stage is 'Staging efímero del padrón RNPJSFL. Nunca alimenta analítica hasta superar el quality gate y promoción atómica.';
comment on table public.aml_osfl_registry_ingest_run is 'Auditoría de cada intento de carga del padrón nacional OSFL: fuente, hash, cobertura, calidad y resultado.';
comment on function public.aml_generate_osfl_registry_probable_links_0940(integer) is 'Genera candidatos fuzzy territoriales para revisión. MATCH_PROBABLE nunca consolida identidad automáticamente.';
