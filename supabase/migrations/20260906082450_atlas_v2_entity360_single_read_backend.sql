...
with identity_row as (
  ...
  from (
    select *
    from public.aml_sii_entity_year h
...
comment on function public.atlas_v2_entity360_read(text,text) is 'ATLAS v2 Entidad 360 single-read backend. Security invoker; preserves caller RLS and collapses executive sources into one network request.';
