-- ATLAS v2 · Entity 360 legacy-parity read model
-- Recupera, bajo el contrato v2 y la allowlist vigente, los mismos cortes gobernados
-- que alimentaban el Expediente Analítico 360 previo. No monta runtime legacy.
...
begin
  ...
  if v_rut = '' then
    select upper(regexp_replace(coalesce(r.rut,''),'[^0-9Kk]','','g'))
      into v_rut
  ...
  select coalesce(jsonb_agg(to_jsonb(m) order by m.contribution desc nulls last, m.mark_id), '[]'::jsonb)
    into v_marks
  from (
    select *
  ...
  select coalesce(jsonb_agg(to_jsonb(p) order by p.commercial_year desc), '[]'::jsonb)
    into v_peers
  from (
    select *
  ...
  select coalesce(jsonb_agg(to_jsonb(s) order by s.source_event_date desc nulls last), '[]'::jsonb)
    into v_sanction_resolution
  from (
    select *
    from public.aml_sanction_identity_resolution
    where resolved_entity_id = p_entity_id
    order by source_event_date desc nulls last
    limit 20
  ) s;

  select coalesce(jsonb_agg(to_jsonb(l) order by l.refreshed_at desc nulls last), '[]'::jsonb)
    into v_links
  from (
    select *
...
revoke all on function atlas_v2_private.entity_dossier_legacy_parity(text,text) from public;
grant execute on function atlas_v2_private.entity_dossier_legacy_parity(text,text) to authenticated, service_role;

create or replace function public.atlas_v2_entity_intelligence_read(
  p_entity_id text,
  p_rut text default null
)
returns jsonb
...
as $$
  select jsonb_build_object(
    ...
    'entity_id',p_entity_id,
    'resolved_rut',coalesce(nullif(btrim(p_rut),''),base->>'resolved_rut'),
    'generated_at',now(),
    'base',base,
    'semantics',jsonb_build_object(
    ...
    'dossier',atlas_v2_private.entity_dossier_legacy_parity(p_entity_id,p_rut)
  ) || atlas_v2_private.entity_context_extensions_final(p_entity_id,p_rut)
  from (select public.atlas_v2_entity360_read(p_entity_id,p_rut) as base) q;
$$;
