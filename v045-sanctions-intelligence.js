...
(function sanctionsIntelligenceLayer(){
 ...
 function subjectForEvent(e){if(!window.V034?.data?.subjects)return null;return V034.data.subjects.find(s=>s.events?.some(x=>x.id===e.id));}
 ...
 function gauge(value,cls=''){const x=4+(clamp(value)/100)*92;return `<svg viewBox="0 0 100 18" preserveAspectRatio="none" aria-hidden="true"><line class="track" x1="4" y1="9" x2="96" y2="9"/><line class="fill" x1="4" y1="9" x2="${x}" y2="9"/></svg>`}
 function compute(D){
   ...
   const so=subjects.filter(s=>s.isSO).length,potential=subjects.filter(s=>!s.isSO&&s.potential?.yes).length;
   return {events,subjects,years,yearly,last,prev,delta,topSup,supShare,recurring,recShare,threePlus,ufCoverage,uf,laft,laftShare,so,potential};
 }
